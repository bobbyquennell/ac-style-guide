...
function generate(paths) {
  ...
  var componentData = getDirectories(paths.components).map(function(
    ...
    try {
      ...
      errors.push(
        "An error happened when attempting to generate metadata for " +
          componentName +
          ". " +
          error
      );
...
function getExampleData(examplesPath, componentName) {
  try {
    ...
    console.log(
      "An error happened when try to getExampleData for: " +
        componentName +
        ". " +
        "examplesPath: " +
        examplesPath +
...
function getExampleFiles(examplesPath, componentName) {
  ...
  try {
    ...
    console.log(
      chalk.red(`No examples found for ${componentName}. info: ${error}`)
    );
  }
  return exampleFiles;
}
